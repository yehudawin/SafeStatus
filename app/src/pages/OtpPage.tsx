...
export default function OtpPage() {
  ...
  const verify = async () => {
    ...
    try {
      ...
      hapticError()
      setLoading(false)
      toast.show('שגיאת תקשורת. נסו שוב.', 'error')
    }
  }

  const resend = async () => {
    try {
      const { error: resendError } = await supabase.auth.signInWithOtp({ phone: e164 })
      if (resendError) {
        toast.show('שגיאה בשליחת קוד חדש', 'error')
        return
      }
      setTimeLeft(45)
  ...
  if (!phone || !e164) return <Navigate to="/login" replace />
