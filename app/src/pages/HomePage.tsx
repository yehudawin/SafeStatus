...
export default function HomePage() {
  ...
  useEffect(() => {
    const ch = supabase
      ...
      .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'profiles' }, () => fetchContacts())
      .subscribe()
  ...
  const sendStatusRequest = async (toUserId: string) => {
    if (!user) return
    await supabase.from('status_requests').insert({ from_user_id: user.id, to_user_id: toUserId })
    ...
    <AppShell alertBadge={!!alert}>
      ...
      <header className="bg-[#1F2937]/90 backdrop-blur-md sticky top-0 z-50 border-b border-[#374151] px-5 py-4 flex justify-between items-center h-[72px]">
        ...
        <button
          ...
          className="w-10 h-10 rounded-full bg-[#374151] hover:bg-gray-600 flex items-center justify-center text-[#F9FAFB] transition-colors relative"
        >
      ...
      <main className="flex-1 overflow-y-auto pb-24 relative">
          ...
          <div className="grid grid-cols-2 gap-4">
            <button
              ...
              className={`group relative flex flex-col items-center justify-center p-6 bg-[#1F2937] border rounded-2xl transition-all duration-300 h-40 ${
                profile?.status === 'safe'
                  ? 'border-[#10B981] shadow-[0_0_20px_rgba(16,185,129,0.3)] bg-[#10B981]/10'
            ...
            <button
              onClick={() => updateStatus('in_shelter')}
              disabled={statusUpdating}
              className={`group relative flex flex-col items-center justify-center p-6 bg-[#1F2937] border rounded-2xl transition-all duration-300 h-40 ${
                profile?.status === 'in_shelter'
                  ? 'border-[#F59E0B] shadow-[0_0_20px_rgba(245,158,11,0.3)] bg-[#F59E0B]/10'
        ...
        <section className="px-5 mt-2">
          <div className="flex justify-between items-end mb-4">
            ...
            <button onClick={fetchContacts} className="text-xs bg-[#1F2937] border border-[#374151] px-2 py-1 rounded-lg text-[#D1D5DB] hover:text-[#F9FAFB] transition-colors">
      ...
      {selectedContact && (
        <div className="absolute inset-0 z-[60] flex items-end bg-black/60 backdrop-blur-sm animate-fade-in" onClick={() => setSelectedContact(null)}>
          <div className="bg-[#1F2937] w-full rounded-t-[32px] border-t border-[#374151] shadow-2xl p-6 pb-12 relative max-h-[90%] overflow-y-auto animate-slide-up" onClick={e => e.stopPropagation()}>
            ...
            <button onClick={() => setSelectedContact(null)} className="absolute top-6 left-6 text-[#D1D5DB] hover:text-[#F9FAFB] p-2"><i className="fa-solid fa-xmark text-xl" /></button>
            ...
            <button onClick={() => { sendStatusRequest(selectedContact.friend_id); setSelectedContact(null) }}
              className="w-full bg-[#3B82F6] hover:bg-blue-600 text-white font-bold py-4 rounded-xl shadow-lg shadow-blue-900/20 transition-all active:scale-[0.98] flex items-center justify-center gap-2">
